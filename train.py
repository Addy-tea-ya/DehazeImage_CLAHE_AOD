...
def load_data(cfg):
    data_transform = transforms.Compose([
        transforms.Resize([413,550]),
        transforms.ToTensor()
    ])
    train_haze_dataset = HazeDataset(cfg.ori_data_path, cfg.haze_data_path, data_transform,"RESIDE")
    ...
    val_haze_dataset = HazeDataset(cfg.val_ori_data_path, cfg.val_haze_data_path, data_transform,"RESIDE")
...
def main(cfg):
    ...
    for epoch in range(cfg.epochs):
        for step, (ori_image, haze_image) in enumerate(train_loader):
            ...
            #ori_image = clahe.apply((ori_image).numpy())
            #haze_image = clahe.apply((haze_image).numpy())
